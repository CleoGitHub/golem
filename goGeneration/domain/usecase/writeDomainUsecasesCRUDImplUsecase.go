...
func (g *GenerationUsecaseImpl) WriteDomainUsecasesCRUDImplUsecase(ctx context.Context, domain *model.Domain, usecaseCRUDImplementation *model.Struct, path string) error {
	...
	f, err := os.Create(filepath + "/" + stringtool.LowerFirstLetter(usecaseCRUDImplementation.Name) + ".go")
	if err != nil {
		return merror.Stack(err)
	}
	defer f.Close()
	...
	str, err := stringifier.StringifyStructUsecase(ctx, pkgManager, usecaseCRUDImplementation)
	if err != nil {
		return merror.Stack(err)
	}
